function createWorkoutSection(title, items) {
    ...
    items.forEach(item => {
        if (typeof item === 'object' && item.complex) {
            // Handle complex exercises
            const complexTitle = document.createElement('div');
            complexTitle.className = 'complex-title';
            complexTitle.textContent = item.title;
            list.appendChild(complexTitle);
...
function formatWorkout(workout) {
    ...
    if (workout.meta) {
        ...
        meta.textContent = workout.meta;
        container.appendChild(meta);
    }

    // Add warmup section if it exists
    if (workout.warmup) {
        container.appendChild(createWorkoutSection('Warm-up', workout.warmup));
    }
    
    // Add main workout section if it exists
    if (workout.workout) {
        container.appendChild(createWorkoutSection('Workout', workout.workout));
    }
    
    // Add finisher section if it exists
    if (workout.finisher) {
        container.appendChild(createWorkoutSection('Finisher', workout.finisher));
    }
